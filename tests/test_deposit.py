...
from tests.ssz import DepositData, DepositDataList, hash_tree_root
...
def test_deposit_amount(registration_contract, success, amount, assert_tx_failed, deposit_input):
...
def test_deposit_event_log(registration_contract):
    log_filter = registration_contract.events.DepositEvent.create_filter(from_block="latest")
...
def test_deposit_tree(registration_contract, w3):
    log_filter = registration_contract.events.DepositEvent.create_filter(from_block="latest")
